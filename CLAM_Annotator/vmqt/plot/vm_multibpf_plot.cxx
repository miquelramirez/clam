...
namespace CLAM
{
	namespace VM
	{
		...
		void MultiBPFPlot::add_bpf(const QString& key, BPF* bpf)
		{
			if(exist_key(key))
			{
			...
			{
				wp_plot->add_renderer(key, new BPFEditor());
		...
		void MultiBPFPlot::set_colors(const QString& key, const Color& cline, const Color& chandler)
		{
			if(!exist_key(key)) return;
			static_cast<BPFEditor*>(wp_plot->get_renderer(key))->set_colors(cline,chandler);
		}
		...
		void MultiBPFPlot::set_flags(const QString& key, int flags)
		{
			...
			if(!exist_key(key)) return;
			static_cast<BPFEditor*>(wp_plot->get_renderer(key))->set_flags(flags);
			wp_bpf_keys[key]=static_cast<BPFEditor*>(wp_plot->get_renderer(key))->get_flags();
		}
		...
		void MultiBPFPlot::backgroundWhite()
		{
			...
			static_cast<Grid*>(wp_plot->get_renderer("grid"))->set_grid_color(Color(152,102,32));
		}

		void MultiBPFPlot::backgroundBlack()
		{
			SegmentationPlot::backgroundBlack();
			static_cast<Grid*>(wp_plot->get_renderer("grid"))->set_grid_color(Color(255,255,0));
		}

		void MultiBPFPlot::showGrid(bool show)
		{
			static_cast<Grid*>(wp_plot->get_renderer("grid"))->show_grid(show);
		}

		void MultiBPFPlot::snapToGrid(bool snap)
		{
			static_cast<Grid*>(wp_plot->get_renderer("grid"))->snap_to_grid(snap);
		}
		...
		void MultiBPFPlot::readOnly()
		{
			...
			{
				static_cast<BPFEditor*>(wp_plot->get_renderer(it->first))->set_flags(0);
			}
		...
		void MultiBPFPlot::setCurrentBPF(QString key)
		{
			...
			static_cast<BPFEditor*>(wp_plot->get_renderer(key))->set_flags(wp_bpf_keys[key]);
			wp_current_bpf = key;
