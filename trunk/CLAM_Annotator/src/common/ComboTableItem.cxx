...
ComboTableItem::ComboTableItem( QTable *table, std::vector<QStringList> &list, bool editable )
			   ...
			   const QRect &cr, bool selected )
{
	...
	p->save();
	QRect textR = table()->style().querySubControlMetrics(QStyle::CC_ComboBox, fakeCombo,QStyle::SC_ComboBoxEditField);
	int align = alignment(); 
	p->drawText( textR, wordWrap() ? ( align | WordBreak ) : align, *entries[0].at( current ) );
	p->restore();
    }
    else
    {
