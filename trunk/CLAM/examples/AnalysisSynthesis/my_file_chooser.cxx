...
static Fl_Pixmap *file_pixmap = new Fl_Pixmap(file_xpm);
static Fl_Pixmap *folder_pixmap = new Fl_Pixmap(folder_xpm);

static void default_callback(const char*) {}
static void (*current_callback)(const char*) = default_callback;
void my_file_chooser_callback(void (*cb)(const char*)) {
  current_callback = cb ? cb : default_callback;
}

static void my_align(Fl_Widget* w)
{
...
void FCB::item_draw(void* v, int x, int y, int, int h) const {
  ...
  const char* e = end_of_name(p);
	Fl_Pixmap* pixmap = file_pixmap;
	
  if (checkdir(p, (char*)e)) 
	{
  ...
  fl_font(textfont(), textsize());
	pixmap->draw(x+2,y);
  fl_draw(p->d_name, e-p->d_name, x+20, y+h-3);
}
...
const char* my_file_chooser_button;
const char* my_file_chooser_data;
		...
		WIDTH_BROWSER, HEIGHT_BROWSER)
{
  ...
  if (my_file_chooser_button) {
    obj = extra_button = new Fl_Button(WIDTH_SPC,but_y,WIDTH_BUT,HEIGHT_BUT,my_file_chooser_button);
		my_align(obj);
    obj->callback(dir_cb, (void*)my_file_chooser_button);
    but_y += HEIGHT_BUT;
  }

  normal_button = new Fl_Button(WIDTH_SPC, but_y, WIDTH_BUT, HEIGHT_BUT, "");
	my_align(normal_button);
...
char* my_file_chooser(const char* message, const char* pat, const char* fname)
{
  static FCW* f; if (!f) f = new FCW();
  f->ok_button->label(fl_ok);
  f->cancel_button->label(fl_cancel);
  if (extra_button) {
    extra_button->label(my_file_chooser_button);
    extra_button->user_data((void*)(my_file_chooser_data ? my_file_chooser_data : my_file_chooser_button));
  }
