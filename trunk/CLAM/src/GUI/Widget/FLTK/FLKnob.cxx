...
void Fl_Knob::draw() {
	...
	{
		side = hh;
		ox = int(ox + (ww - side) / 2.0); 
	}
	else
	{
		side = ww;
		oy = int(oy + (hh - side) / 2.0); 
	}
	side = w() > h () ? hh:ww;
	int dam = damage();
		...
		case FL_DRAG:
			{
				...
				if (!mx && !my) return 1;
				double angle = 270.0-atan2((double)-my, (double)mx)*180.0/M_PI;
...
void Fl_Knob::draw_scale(const int ox,const int oy,const int side) {
	double x1,y1,x2,y2,rds,cx,cy,ca,sa;
	...
	else
	{
		...
		{
			double a_step = (10.0*3.14159/6.0) / double(nb_dec);
			double a_orig = -(3.14159/3.0) + double(k) * a_step;
...
void Fl_Knob::draw_cursor(const int ox,const int oy,const int side) {
	double rds,cur,cx,cy;
	double angle;
	
	rds = (side - 20.0) / 2.0;
	cur = _percent * rds / 2.0;
	cx = ox + side / 2.0;
	cy = oy + side / 2.0;
