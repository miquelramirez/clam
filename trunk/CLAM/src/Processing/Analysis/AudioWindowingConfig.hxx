...
#ifndef _AudioWindowingConfig_
#define _AudioWindowingConfig_
...
class AudioWindowingConfig:public ProcessingConfig
{
	friend class AudioWindowing;

	DYNAMIC_TYPE_USING_INTERFACE (AudioWindowingConfig,8,ProcessingConfig);
	DYN_ATTRIBUTE(0,public,WindowGeneratorConfig, WindowGenerator);
	DYN_ATTRIBUTE(1,public,CircularShiftConfig, CircularShift);
	DYN_ATTRIBUTE(2,public,int, prHopSize);
	DYN_ATTRIBUTE(3,public,int, prZeroPadding);
	DYN_ATTRIBUTE(4,public,int, prSamplingRate);
	DYN_ATTRIBUTE(5,public,int, prFFTSize);
	//xamat: added these so they can be accessible from a configurator
	DYN_ATTRIBUTE(6,public,int, prWindowSize);
	DYN_ATTRIBUTE(7,public,EWindowType, prWindowType);
