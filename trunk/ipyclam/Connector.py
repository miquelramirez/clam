...
class Connector(object):
	...
	In = "In"
	Out = "Out"
	def __init__(self, networkProxy, name = "inport1", kind=Port, direction=In, index=1, type=None):
...
class ConnectorTests(unittest.TestCase):
	...
	def test_gettingKindPort(self):
		port = Connector(definition.proxy(), name="inport1", kind=Port, direction=In, index=2, type="tipus1")
		self.assertEqual(port.kind, "Port")
	def test_gettingKindControl(self):
		port = Connector(definition.proxy(), name="inport1", kind=Control, direction=In, index=2, type="tipus1")
	...
	def test_gettingDirectionOut(self):
		port = Connector(definition.proxy(), name="inport1", kind=Port, direction=Out, index=2, type="tipus1")
	...
	def test_gettingIndex(self):
		port = Connector(definition.proxy(), name="inport1", kind=Port, direction=In, index=1, type="tipus1")
