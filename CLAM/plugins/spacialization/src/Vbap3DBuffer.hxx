...
class Vbap3DBuffer : public CLAM::Processing
{
public:
...
public:
	const char* GetClassName() const { return "Vbap3DBuffer"; }
	Vbap3DBuffer(const Config& config = Config()) 
		: _lastAzimuth(0)
		, _lastElevation(0)
		, _w("W", this)
...
//std::cout << "\nazimuth, elevation: " << azimuthDegrees << "\t" << elevationDegrees << std::endl;
		...
		CLAM_ASSERT(newTriangle > -1, "Vbap3DBuffer: findTriangle() found no triangle for the given angle!");
