...
#include "DynamicNote.hxx"
#include "DynamicADSR.hxx"
#include "DynamicRegion.hxx"
...
int testXML()
{
	...
	*/
	...
	CXMLStorage* storage = new XMLStorage("XML-Document");
	b.StoreOn(*storage);

	fstream fileout("test.xml", ios::out);

	storage->dumpOn(fileout);
	
	//delete storage;
	CXMLStorage* storage1 = new XMLStorage("XML-Document");
	b.StoreOn(*storage1);


	storage1->dumpOn(fileout);
...
int testDeepCopy()
{
	...
	Component* branchCopy = b.SwallowCopy();
...
typedef CCLAMTmplArray<CCLAMPort> CCLAMPortArray;
...
class CCLAMConcreteProcObj: public CCLAMProcObj
{
public:
...
class CCLAMConcreteProcObj2: public CCLAMProcObj
{
public:
