...
namespace CLAM
{
	...
	void BPFEditorController::SetDialColor(Color c)
	{
	    mDial.SetColor(c);
	}
	...
	void BPFEditorController::UpdatePoint(const TData& x, const TData& y)
	{
		...
		else
		{
			...
			if(!mIsPlaying) mSelectPoint=true;
	...
	void BPFEditorController::Draw()
	{
	    ...
	    mRenderer.SetBounds( GetBound(TData(mView.mLeft)) , GetBound(TData(mView.mRight),false) );
	    mRenderer.Render();
	    mDial.Render();
	    if(mEFlags & CLAM::VM::AllowZoomByMouse)
	    {
