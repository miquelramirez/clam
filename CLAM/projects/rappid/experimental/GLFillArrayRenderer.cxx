...
GLFillArrayRenderer::GLFillArrayRenderer(const c3f &color,
										   unsigned size,
										   TData first,
										   TData last)
...
void GLFillArrayRenderer::ResetXValues()
{
	TData val = mFirst;
...
void GLFillArrayRenderer::ResizeVertexArray( unsigned int new_size )
{
...
void GLFillArrayRenderer::DataTransform( const DataArray& array )
{
	for ( int k = 0; k < mSize; k++ )
		mVertex[2*k+1].y = GLfloat( array[k] ); 
}

void GLFillArrayRenderer::Draw(TData x_start, TData x_width)
{
	...
	glDrawArrays( GL_QUAD_STRIP, 0, len*2-1);
}

void GLFillArrayRenderer::Draw()
{
	glColor3fv(&mColor[0]);
	glVertexPointer(2,GL_FLOAT,1,&mVertex[0]);
	glDrawArrays( GL_QUAD_STRIP, 0, mVertex.Size() );
}
